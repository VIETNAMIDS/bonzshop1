                  ...
                  <div key={u.user_id} className={`glass rounded-xl p-4 ${u.isRootAdmin ? 'border border-orange-500/50' : ''} ${u.isBanned ? 'opacity-60 border-destructive/50' : ''} ${isSelected ? 'border-primary/50 bg-primary/5' : ''}`}>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-1">
                        <Button variant="ghost" size="icon" className="h-9 w-9" onClick={() => setDetailUser(u)}>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${
                  detailUser.isRootAdmin ? 'bg-gradient-to-br from-yellow-400 to-orange-500' : 'bg-primary/20'
                }`}>
                  {detailUser.isRootAdmin ? <Crown className="w-5 h-5 text-white" /> : <User className="w-5 h-5 text-primary" />}
                </div>
                <div>
                  <p className="font-semibold">{detailUser.display_name || 'Chưa đặt tên'}</p>
                  <p className="text-xs text-muted-foreground">{detailUser.user_id}</p>
                </div>
              </div>
              ...
              <div className="grid gap-3 text-sm">
                <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{detailUser.email || 'Chưa có'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{detailUser.phone || 'Chưa cập nhật'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{detailUser.ip_address || 'Chưa có dữ liệu'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{detailUser.device_name || [detailUser.browser, detailUser.os].filter(Boolean).join(' • ') || 'Chưa có dữ liệu'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                  ...
                  <div>
                    <p className="text-xs text-muted-foreground">Trình duyệt / HĐH</p>
                    <p className="font-medium">{[detailUser.browser, detailUser.os].filter(Boolean).join(' / ') || 'Chưa có'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{new Date(detailUser.created_at).toLocaleString('vi-VN')}</p>
                  </div>
                </div>

                {detailUser.last_active_at && (
                  <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                    <Globe className="w-4 h-4 text-primary shrink-0" />
                    <div>
                      <p className="text-xs text-muted-foreground">Hoạt động lần cuối</p>
                      <p className="font-medium">{new Date(detailUser.last_active_at).toLocaleString('vi-VN')}</p>
                    </div>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-0.5">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${
                        detailUser.isRootAdmin ? 'bg-gradient-to-r from-yellow-400/20 to-orange-500/20 text-orange-500'
                          : detailUser.isAdmin ? 'bg-yellow-500/20 text-yellow-500' : 'bg-primary/20 text-primary'
                      }`}>
                        {detailUser.isRootAdmin ? 'Admin Gốc' : detailUser.isAdmin ? 'Admin' : 'User'}
                      </span>
                      {detailUser.isSeller && <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-blue-500/20 text-blue-500">Seller</span>}
                      {detailUser.isBanned && <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-destructive/20 text-destructive">Đã ban</span>}
                    </div>
                  </div>
                </div>
              </div>
